...
class TableManager<_Table>::TableManager(const std::string &desc,
...
template<class _Table>
class TableManager<_Table>::~TableManager()
{
//        TRACE_LOG("%s", _desc.c_str());
...
template<class _Table>
std::string TableManager<_Table>:: &desc() const
{
    return _desc;
}
...
template <class _Table>
TableManager<_Table> *
TableManager<_Table>::clone(_TableGroup *p_table_group) const
{
...
template <class _Table>
bool TableManager<_Table>::update(const _IncRecordType &inc_record)
{
    if (NULL == _p_update_strategy) {
